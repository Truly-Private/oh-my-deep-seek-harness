...
- `@truly-private/omdsh-jobs` 将运行中的工作注册为 `ctx.jobs`，并拥有 job id、授权、快照、读取、取消、等待、完成监听器与清理。
- `@truly-private/omdsh-tool-jobs` 暴露 `job_output`、`job_list` 和 `job_kill`，注入完成通知，并提供后台任务的系统提示词指导。
...
`JobRegistry` 是 `@truly-private/omdsh-jobs` 中的 Service Definition；进程内 Service Provider 是 `@truly-private/omdsh-jobs-local` 中的 `LocalJobRegistry`（该拆分记录在[任务注册表约定 Agent Note](2026-07-26-job-registry-seam.md)中）。
