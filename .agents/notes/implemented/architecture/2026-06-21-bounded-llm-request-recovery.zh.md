...
`@truly-private/omdsh-llm` 导出唯一的可 JSON 序列化 `LlmFailure` 载荷：
...
`@truly-private/omdsh-llm-retry` 是监听 `agent/request-error` 的函数插件。它不引入服务或新的循环分支；agent-loop 包仅会更改通过现有失败步骤恢复控制流携带的数据。
