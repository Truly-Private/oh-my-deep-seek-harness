...
1. `@truly-private/omdsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@truly-private/omdsh-web-search-exa`, `@truly-private/omdsh-web-search-perplexity`, `@truly-private/omdsh-web-search-deepseek`, and `@truly-private/omdsh-web-fetch-http`.
3. `@truly-private/omdsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@truly-private/omdsh-tool-web  --depends on-->  @truly-private/omdsh-web  <--depends on--  @truly-private/omdsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @truly-private/omdsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @truly-private/omdsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @truly-private/omdsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@truly-private/omdsh-web-search-exa"] -->|registerSearchProvider| web["@truly-private/omdsh-web / ctx.web"]
  perplexity["@truly-private/omdsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@truly-private/omdsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@truly-private/omdsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@truly-private/omdsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@truly-private/omdsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@truly-private/omdsh-tool-web` depends on `@truly-private/omdsh-web`, `@truly-private/omdsh-tools`, `@truly-private/omdsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@truly-private/omdsh-web'
...
```yaml
- id: web
  name: '@truly-private/omdsh-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@truly-private/omdsh-web-search-exa'

- id: web-search-perplexity
  name: '@truly-private/omdsh-web-search-perplexity'

- id: web-search-deepseek
  name: '@truly-private/omdsh-web-search-deepseek'
