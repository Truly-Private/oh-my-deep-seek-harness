...
1. `@truly-private/omdsh-web`（`packages/web/web`）拥有 `ctx.web`、提供方注册、提供方选择、共享的请求/结果词汇，以及 web 特有的错误。
2. 提供方包实现具体后端并向 `ctx.web` 注册能力，例如 `@truly-private/omdsh-web-search-exa`、`@truly-private/omdsh-web-search-perplexity`、`@truly-private/omdsh-web-search-deepseek` 和 `@truly-private/omdsh-web-fetch-http`。
3. `@truly-private/omdsh-tool-web`（`packages/web/tool-web`）拥有面向模型的 `web_search` 和 `web_fetch` 工具 schema、提示词段落、参数校验、结果格式化，以及通过 `ctx.web` 实现的工具展示。
...
`@truly-private/omdsh-web` 仅依赖 Cordis 和底层 harness 支持。它声明 `ctx.web`、提供方接口、请求/结果类型、提供方可用性约定和错误码。它不导入工具、agent（智能体）、会话、LLM 或提供方包。
...
`@truly-private/omdsh-tool-web` 依赖 `@truly-private/omdsh-web`、`@truly-private/omdsh-tools`、`@truly-private/omdsh-system-prompt` 和 Cordis。它从不导入具体的提供方包。
