...
| 包 | 角色 |
|---|---|
| `@truly-private/omdsh-spill` | 接口：`ctx.spillStore`、词汇类型，不包含存储实现。 |
| `@truly-private/omdsh-spill-local` | 本地后端：在宿主文件系统中提供私有、会话作用域的文件存储。 |
| `@truly-private/omdsh-spill-policy` | 工具结果策略插件：包装分发后的最终文本结果，并以保留预览和 spill 定位符替换超大结果。 |
...
- `@truly-private/omdsh-output-retention` 负责预览机制（`TextRetainer`、`ItemRetainer` 和省略元数据）。
- `@truly-private/omdsh-spill` 负责保存最终文本，并返回定位符与检索提示。
- `@truly-private/omdsh-spill-policy` 在工具流水线中应用默认的最终结果策略，将前两者组合起来。
