...
`@truly-private/omdsh-scope` 提供与键类型无关的 `store.ts` 实现模块。该包继续将 Cordis 和 `@truly-private/omdsh-invariants` 列为对等依赖（peer dependency），其不变量配套模块保持不变。包根导出四个存储符号：`ScopeLayer`、`ScopedLayers`、`NamedEntries` 和 `AnonymousEntries`。`EntryValues` 仍是内部接口，`store.ts` 不是包子路径。
