...
`@truly-private/omdsh-invariants` 是与产品无关的 Cordis 服务插件，注册 `ctx.invariants`。它只负责配置、注册唯一性、子 fiber 生命周期和带包归属的失败；不导入 session、agent、scope 或 agent-loop 包，也不包含这些包的检查。
...
| 伴随入口 | 注册名 | 所属检查 |
|---|---|---|
| `@truly-private/omdsh-session/invariant` | `@truly-private/omdsh-session` | 会话序列、轮次/步骤包围关系和同一步骤的调用/结果轨迹 |
| `@truly-private/omdsh-agent/invariant` | `@truly-private/omdsh-agent` | agent 状态转换 |
| `@truly-private/omdsh-scope/invariant` | `@truly-private/omdsh-scope` | 作用域事件载体的存在性与主体一致性 |
| `@truly-private/omdsh-agent-loop/invariant` | `@truly-private/omdsh-agent-loop` | 模型请求重建 |
