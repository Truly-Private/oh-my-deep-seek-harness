...
每个加载 bash 执行器的组合都同时加载 `@truly-private/omdsh-subprocess-local`：CLI（命令行界面）、各示例、Python 捆绑运行时以及各内联测试配置。
