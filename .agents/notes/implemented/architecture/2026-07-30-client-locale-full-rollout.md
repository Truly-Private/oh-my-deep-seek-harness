...
**Test and e2e doctrine**: `makeTranslate(...dicts)` (dsh-client-test-runtime) mirrors the service lookup chain (first-dict-wins, key fallback, `{name}` interpolation); component specs stub the `t` seat with it, typed against real props seats. Web e2e opens through `newEnglishPage` for English scenarios, while Chinese scenarios explicitly seed the Host locale preference before browser boot; browser language is not a product-default input ([Host-backed Web preferences](../bug-fix/2026-08-06-host-backed-web-preferences.md)).
