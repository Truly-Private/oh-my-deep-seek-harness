...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@truly-private/omdsh-sdk-client`, `@truly-private/omdsh-sdk-protocol`, and the wire identity `deepseek-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. DeepSeek Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
|---|---|---|
| `@truly-private/omdsh-jsonrpc` | `@truly-private/omdsh-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@truly-private/omdsh-sdk-client`, `@truly-private/omdsh-sdk-protocol`, and `deepseek-harness-sdk-runtime`. Exclude `@deepseek-ai/create-sdk`, `@truly-private/omdsh-scripts`, `@truly-private/omdsh-helper`, and `@truly-private/omdsh-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@truly-private/omdsh-bash`, `ctx.bash` | `@truly-private/omdsh-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@truly-private/omdsh-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@truly-private/omdsh-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@truly-private/omdsh-pty`, `ctx.pty`, `PtyService` | `@truly-private/omdsh-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@truly-private/omdsh-pty-local`, `LocalPtyBackend` | `@truly-private/omdsh-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@truly-private/omdsh-tool-pty` | `@truly-private/omdsh-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@truly-private/omdsh-lsp-local` | `@truly-private/omdsh-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@truly-private/omdsh-tasks`, `ctx.tasks`, `TaskService` | `@truly-private/omdsh-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@truly-private/omdsh-tasks-local`, `LocalTaskService` | `@truly-private/omdsh-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@truly-private/omdsh-tool-tasks` | `@truly-private/omdsh-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@truly-private/omdsh-client-ui-task`, `client/ui-task/` | `@truly-private/omdsh-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@truly-private/omdsh-client-ui-slash`, `ui-slash/` | `@truly-private/omdsh-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@truly-private/omdsh-agent-tool-mode`, plugin `tool-mode` | `@truly-private/omdsh-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@truly-private/omdsh-permission`, `ctx.permission`, `PermissionService` | `@truly-private/omdsh-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@truly-private/omdsh-client-ui-permission` | `@truly-private/omdsh-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@truly-private/omdsh-user-interaction`, `user-interaction/` | `@truly-private/omdsh-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@truly-private/omdsh-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@truly-private/omdsh-type-meta` | `typert/protocol/`, `@truly-private/omdsh-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@truly-private/omdsh-tools` and `ctx.tools`. Keep `@truly-private/omdsh-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@truly-private/omdsh-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@truly-private/omdsh-workspace-context`, `context/workspace-context/` | `@truly-private/omdsh-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `session/user-id/`, `@truly-private/omdsh-user-id` | `identity/anonymous-user-id/`, `@truly-private/omdsh-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@truly-private/omdsh-environment` | `util/launch-environment/`, `@truly-private/omdsh-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
...
| Former | Current | Reason |
|---|---|---|
| `@truly-private/omdsh-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@truly-private/omdsh-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@truly-private/omdsh-workflow` and workflow events and tools. |
| `@truly-private/omdsh-workflow-workerthread`, `WorkerWorkflowEngine` | `@truly-private/omdsh-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@truly-private/omdsh-goal-session`, `goal/goal-session/` | `@truly-private/omdsh-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@truly-private/omdsh-compact`, `ctx.compact`, `CompactService` | `@truly-private/omdsh-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@truly-private/omdsh-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@truly-private/omdsh-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@truly-private/omdsh-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@truly-private/omdsh-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@truly-private/omdsh-settings-local`, `SettingsLocal` | `@truly-private/omdsh-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@truly-private/omdsh-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@truly-private/omdsh-session-title-first-message-llm`, cadence `first-message` | `@truly-private/omdsh-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@truly-private/omdsh-session-title-all-messages-llm`, cadence `all-user-messages` | `@truly-private/omdsh-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@truly-private/omdsh-code-runtime-worker`, `WorkerCodeRuntime` | `@truly-private/omdsh-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@truly-private/omdsh-fs-policy` | `@truly-private/omdsh-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@truly-private/omdsh-skill-local`, `LocalSkillProvider`, provider id `local` | `@truly-private/omdsh-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@truly-private/omdsh-subagent-spawn`, `SpawnProvider` | `@truly-private/omdsh-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@truly-private/omdsh-subagent-fork`, `ForkProvider` | `@truly-private/omdsh-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@truly-private/omdsh-subagent-inprocess`, `subagent-inprocess/` | `@truly-private/omdsh-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
...
| `@truly-private/omdsh-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@truly-private/omdsh-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@truly-private/omdsh-subagent-dsh-sdk`, its provider id `dsh-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@truly-private/omdsh-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@truly-private/omdsh-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@truly-private/omdsh-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@truly-private/omdsh-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@truly-private/omdsh-timeout-policy` | `@truly-private/omdsh-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@truly-private/omdsh-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@truly-private/omdsh-paths` | `util/home-paths/`, `@truly-private/omdsh-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@truly-private/omdsh-retention` | `util/output-retention/`, `@truly-private/omdsh-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@truly-private/omdsh-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@truly-private/omdsh-frontend-static` | `@truly-private/omdsh-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@truly-private/omdsh-jsonrpc-demo` | `@truly-private/omdsh-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@truly-private/omdsh-frontend` | `@truly-private/omdsh-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |
...
| `@truly-private/omdsh-client-ui-model` | `@truly-private/omdsh-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@truly-private/omdsh-client-ui-models` | `@truly-private/omdsh-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@truly-private/omdsh-client-ui-plugin-config`, `client/ui-plugin-config/` | `@truly-private/omdsh-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@truly-private/omdsh-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@truly-private/omdsh-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@truly-private/omdsh-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@truly-private/omdsh-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@truly-private/omdsh-client-ui-question` | `@truly-private/omdsh-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@truly-private/omdsh-client-ui-command`, `ui-command/` | `@truly-private/omdsh-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@truly-private/omdsh-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@truly-private/omdsh-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@truly-private/omdsh-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@truly-private/omdsh-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `@truly-private/omdsh-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@truly-private/omdsh-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
