...
Downstream browser builds again reject undeclared workspace value imports, while vendored framework libraries remain usable. The package identity assertions and locale fixtures agree with the downstream runtime, the base profile can resolve its selected 9Router route, the complete coverage gate isolates the real PTY lifecycle without removing it from coverage, and the Web snapshot pool preserves the client artifact digest while waiting for durable UI state. A fresh native ARM64 Ubuntu OrbStack container, with Node, Python, Bun, and pnpm installed through Proto, completed the frozen install, host library build, both real PowerShell PTY tests, and the targeted assembled snapshot. These corrections do not promote the upstream intake beyond `candidate` or authorize a release.
