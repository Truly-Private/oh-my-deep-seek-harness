...
当写入前检查证明前台进程组正在等待 stdin 时，提交了输入的 send 只有在轮询观察到该进程组脱离等待或观察到不同的前台进程组之后，才能通过静默结算。只有在出现前台活动、更早的可打印输出，或同一回调中标记之前的可打印文本后，提示符标记才成为当前 send 的证据。来自前一 send 的延迟标记与提示符不能完成其后继 send。`TerminalSendRequest.expectedPromptText` 让拥有替换提示符的消费方声明确切的可打印值；持久 PowerShell 工具会在初始化、命令与轮询 send 上提供其私有提示符。只轮询的 send 与没有已证明写入前等待的提供方保留现有静默回退。
