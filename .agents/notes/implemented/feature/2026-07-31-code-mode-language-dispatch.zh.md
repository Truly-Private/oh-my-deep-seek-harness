...
`code-mode.ts` 只依赖运行时 Service Definition（`@truly-private/omdsh-code-runtime`），绝不依赖具体后端；分发在运行时按 `runtime.language` 进行。因此工具层独立于 Python 协议和后端——它只需要服务的 `language` 字段。
