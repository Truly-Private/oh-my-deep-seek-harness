...
| 落点 | `@truly-private/omdsh-anonymous-user-id`，由 OTel 后端、`/feedback` 与直连 DeepSeek 请求共享 | 消费方共用同一存储契约，且不依赖导出后端 |
...
| 挂进 `@truly-private/omdsh-home-paths` | paths 是纯路径计算零 IO；带持久化的身份能力会污染包边界 |
