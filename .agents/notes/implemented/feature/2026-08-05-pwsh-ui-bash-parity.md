...
**Import `parseExitStatus` from `@truly-private/omdsh-tool-bash/src/render.ts`.** Rejected: workspace imports stay external in the built bundles, so `tool-pwsh` would gain a hard runtime dependency on `tool-bash` in every consumer closure (including compositions that deliberately mount the pwsh twin without bash), and a sibling tool depending on its twin for one function inverts the package relationship. The seam move keeps the shared contract on a package both tools already depend on.

**A new dedicated presentation package (e.g. `@truly-private/omdsh-shell-present`).** Rejected: a new package costs manifests, module-graph/catalog regeneration, and README surface for a single pure function; `@truly-private/omdsh-shell` is already in both tools' closures and already owns the `ShellRunResult` facts the parse reconstructs.
...
- `parseExitStatus` becomes public contract surface on `@truly-private/omdsh-shell`; `dsh-tool-bash/src/render.ts` keeps re-exporting it, so no bash-tool consumer changes.
