...
**从 `@truly-private/omdsh-tool-bash/src/render.ts` 导入 `parseExitStatus`。** 否决：workspace 导入在构建产物中保持外部引用，因此 `tool-pwsh` 会在每个消费方闭包中新增对 `tool-bash` 的硬运行时依赖（包括刻意只挂 pwsh 孪生、不挂 bash 的组合），且兄弟工具为一个函数而依赖其孪生会颠倒包间关系。seam 迁移把共享约定放在两个工具本就依赖的包上。

**新建专用呈现包（如 `@truly-private/omdsh-shell-present`）。** 否决：为一个纯函数新建包要付出 manifest（元数据清单）、module-graph/目录再生成与 README 内容的成本；`@truly-private/omdsh-shell` 已在两个工具的闭包中，且已拥有该解析重建的 `ShellRunResult` 事实。
...
- `parseExitStatus` 成为 `@truly-private/omdsh-shell` 公开约定的一部分；`dsh-tool-bash/src/render.ts` 继续再导出它，bash 工具消费方零改动。
