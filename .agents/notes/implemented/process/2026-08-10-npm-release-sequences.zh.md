...
下游 scope 与 dsh access 的陈述由[下游 npm scope 与首次公开发布](2026-08-20-downstream-npm-scope.md)取代。发布族拓扑与产物流仍然有效。
...
`packages/*/*` 与 `apps/*` 组成 `@truly-private/omdsh` 的运行面；`vendor/*` 是九个 rescope 过的 Cordis 框架包，各自带着上游的版本号；`native/landlock-run/packages/*` 是 Linux 平台包，有自己的 workflow。三组的版本基线、变更节奏和构建要求都不同：dsh 随产品迭代，vendor 只在同步上游或改动本地修改时才动，native 需要 musl 工具链和逐架构构建。把它们塞进一条发布流水线，等于每次产品发版都要重发框架和原生二进制。
...
|---|---|---|---|---|
| dsh | `packages/*/*` + `apps/*`（`@truly-private/omdsh` 与 `@truly-private/omdsh-web-frontend`） | 全族与 workspace 根共用一个 `0.0.x` | `dsh-v<版本>` | `release.yml` |
...
`scripts/check-workspace-constraints.ts` 要求这个协议，所以新包无法再引入硬写的范围；同理，invariant companion 规则要求 `@truly-private/omdsh-invariants` 用 `workspace:^`。
...
验证还会打一份 Landlock entry 的 tarball——`dsh-sandbox-local` 把它声明为普通 `dependencies`。验证执行默认 npm install，并包含适用于当前平台的可选依赖：Koffi 通过这种方式分发 native binary，而 npm 会跳过平台不兼容的 Landlock binary。因此验证按目录内容读取 tarball，而不是读发布顺序：一个目录可能只装着为满足跨序列依赖而打出来的包，任何发布顺序都不描述它。
...
**按入口闭包挑一部分包发。** 从 `@truly-private/omdsh` 与 `@truly-private/omdsh-web-frontend` 沿 `dependencies` 爬得到 156 个包，比全量少 61 个。但本仓的插件是 `cordis.yml` 按名字挂载的、不是被 import 的：`vendor/cordis-plugin-group` 与 `vendor/cordis-plugin-logger-console` 落在依赖闭包之外，却是运行时必需。照代码依赖挑的失败形态是「消费方装完起不来」，而且要额外持续证明「没漏任何挂载项」。私有 scope 下多出来的包对组织外不可见。`python/`、根 `examples/`、`docs/` 与 `website/` 不是成员。
