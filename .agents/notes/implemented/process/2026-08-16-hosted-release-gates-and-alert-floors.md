...
The required Linux jobs and real-Windows job default to GitHub-hosted runners, while the existing repository-variable switches retain the private failover pools. Hosted concurrency is reduced to match standard runners. The complete native-Windows inventory runs one gate and one coverage worker at a time. Serializing the top-level gates removed their overlap but did not eliminate a repeated Vitest fork exit before the large workflow worker-thread suite completed. Suites that create real nested workers or observe exact-path filesystem events run in the existing process-bound project; the single coverage invocation still aggregates their measurements while a fresh project fork separates them from the broad inventory's accumulated process state. The HMR disposal test waits for the second refresh to enter the serial queue instead of treating a fixed delay as evidence that the filesystem event arrived. Linux, macOS, and Windows serial reference jobs run on pull requests and join the `all checks passed` aggregate. The self-hosted Linux and Windows lanes retain distinct master-push standby job identifiers but run only when the `DSH_CI_STANDBY_DRILLS` repository variable is `enabled`; a public downstream without those runners therefore skips them instead of retaining queued default-branch runs indefinitely.
