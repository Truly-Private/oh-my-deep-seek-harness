...
The required Linux jobs and real-Windows job default to GitHub-hosted runners, while the existing repository-variable switches retain the private failover pools. Hosted concurrency is reduced to match standard runners. The complete native-Windows inventory runs one gate and one coverage worker at a time: a two-gate run completed all test assertions but lost one Vitest fork during instrumented coverage, while the same exact inventory passed under the serialized Windows reference posture. Linux, macOS, and Windows serial reference jobs run on pull requests and join the `all checks passed` aggregate. The self-hosted Linux and Windows lanes retain distinct master-push standby job identifiers but run only when the `DSH_CI_STANDBY_DRILLS` repository variable is `enabled`; a public downstream without those runners therefore skips them instead of retaining queued default-branch runs indefinitely.
