...
The old Agent Note already deferred a separate `@truly-private/omdsh-fs-observation-policy` package. This decision builds that layer and keeps `ctx.fs` close to fsspec-style storage primitives (`info`/`cat`/`open`), without turning it into full fsspec.
...
`@truly-private/omdsh-fs` shrinks to provider text IO plus guarded text mutation:
