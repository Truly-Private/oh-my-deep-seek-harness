...
旧 Agent Note 已经推迟了独立的 `@truly-private/omdsh-fs-observation-policy` 包。本决策构建该层，使 `ctx.fs` 保持接近 fsspec 风格的存储原语（`info`/`cat`/`open`），但不把它变成完整的 fsspec。
...
`@truly-private/omdsh-fs` 收缩为提供方文本 IO 加受保护的文本变更：
...
`@truly-private/omdsh-fs-observation-policy` 是插件，而非服务：它不注册任何 `ctx.*` 键，也不注入任何内容。它拥有不应位于 `FileSystem` 提供方基类上的写入/编辑新鲜度策略和 observed state（否则沙箱/远程后端会继承不该由其承载的面向模型观察策略）。它通过执行器分派的 `fs/*` 事件门禁贡献该策略。
