...
- `@truly-private/omdsh-tui` 负责终端交互式执行。它会在 Loader 启动前拒绝非 TTY 流；`apps/cli/config/base.cordis.yml` 与 `tui.cordis.yml` overlay 拥有完整 coding 组装，PTY 与终端快照覆盖则位于 `apps/cli/tests/`。
...
- [`@truly-private/omdsh-acp-demo`](../../../../packages/examples/acp-demo/README.md) 和 `@truly-private/omdsh-sdk-jsonrpc-server` 负责各自的分帧协议集成。
