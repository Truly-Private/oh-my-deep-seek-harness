...
`SubagentRuntime` is the only public service. It exposes ordinary `start(name, request)`, Task-backed `startContinuable(spec)`, and intent-named `followup(...)`; provider resume dispatch remains private to its continuation manager. The standalone `@truly-private/omdsh-subagent-control` package and `ctx.subagentControl` key are absent; the optional `@truly-private/omdsh-tool-subagent-control` package injects `ctx.subagents` directly.
