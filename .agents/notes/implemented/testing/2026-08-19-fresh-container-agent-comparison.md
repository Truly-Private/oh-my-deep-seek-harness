...
The comparison bench runs Pi baseline and Pi plus DeepSeek Harness in separate disposable Linux containers built from the same digest-pinned base. Moonrepo proto installs every language runtime. Both lanes receive the same Pi version, 9Router `trifecta` route, prompt bytes, empty workspace, resource limits, and named 9Router gateway-key variable. The bench does not use a DeepSeek provider key. The baseline exposes Pi's normal coding tools. The harness lane loads the real Pi extension and invokes `dsh_delegate` directly, avoiding an unrelated outer Pi model turn while preserving the shipped host-to-ACP path.

The agent phase permits network access because model calls and application dependency installation require it. The Docker context excludes common local environment and credential files. Each container reaches the host's 9Router instance through the Docker host gateway, mounts only the output workspace, passes no host credential path or Docker socket, and removes `NINE_ROUTER_API_KEY` before generated commands run. The baseline model runtime retains its in-memory key. The harness ACP runtime reads a mode-0600 credential file from an ephemeral directory outside the workspace and shell sandbox; its child environment does not contain the key. The bench records the commit, dirty state, versions, prompt hash, order, time, and generated-file hashes; the inventory excludes dependency directories and Playwright browser caches because they are execution tooling. A separate credential-free evaluator runs offline with a read-only root filesystem. It runs available tests, requires a production build and local server, checks stable interface hooks, and captures deterministic desktop and mobile screenshots.
