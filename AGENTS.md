...
packages/    @truly-private/omdsh-<pkg> workspaces at packages/<group>/<pkg>/
...
- Every npm package is `@truly-private/omdsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@deepseek-ai/cordis` is a peerDependency (+ dev) of every harness package.
