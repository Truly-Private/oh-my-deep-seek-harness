...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@truly-private/omdsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@truly-private/omdsh-base', '@truly-private/omdsh-web-app'])
    ...
    for (const name of ['@truly-private/omdsh-pwsh-sandbox', '@truly-private/omdsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@truly-private/omdsh-base'])
