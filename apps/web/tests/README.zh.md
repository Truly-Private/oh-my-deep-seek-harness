...
`@truly-private/omdsh-client-web` import `AppWebEntry`、从
`@truly-private/omdsh-client-modules/client` import boot manifest 类型：启动真实 shell 正是该
harness 的用途，且这两个包本来就在 Host 图中。另外，chat 场景从
`@truly-private/omdsh-client-runtime/client` import `conversationContextKey`，因为
