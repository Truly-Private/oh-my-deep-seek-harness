...
import { bootInjections, orderByModuleGraph } from '@truly-private/omdsh-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@truly-private/omdsh-client-modules/client'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@truly-private/omdsh-app-boot')).href) as unknown as BootComposition
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  for (const id of ['@truly-private/omdsh-client-modules', '@truly-private/omdsh-client-runtime']) {
