...
Stdout carries only newline-delimited ACP JSON-RPC. `@truly-private/omdsh-acp-demo` installs no stdout logger; leaf additions must use stderr for diagnostics.

The automation contract — supported methods, baseline prompt content, committed-text output, and the intentionally absent UI surfaces — lives in [`@truly-private/omdsh-acp`](../../packages/acp/acp/README.md).
