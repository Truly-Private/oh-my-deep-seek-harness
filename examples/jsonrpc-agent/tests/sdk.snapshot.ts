 ...
 * REAL `@truly-private/omdsh-sdk-client`, drives one turn over stdio JSON-RPC,
...
import { resolveExampleLaunch } from '@truly-private/omdsh-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@truly-private/omdsh-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@truly-private/omdsh-system-prompt') return []
