...
npm scope: `@truly-private/omdsh-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@truly-private/omdsh-<pkg>`. **Group READMEs own package/ctx-key maps.**
