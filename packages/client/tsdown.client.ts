 ...
 * Everything else under @truly-private/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@truly-private\/omdsh-(host-apiproxy|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@truly-private\/omdsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@truly-private/omdsh-client-runtime/client'
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other @truly-private value import is a build error — a
