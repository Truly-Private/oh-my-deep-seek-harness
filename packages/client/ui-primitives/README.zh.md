...
- **闪光字形是重新绘制的近似版本**：ui-conversation 持有的闪光图标来自字体字形，而本地设计数据无法导出其矢量几何；在获得精确导出路径前，使用手工重建版本代替。产品标志使用所提供的光栅图稿，而不是重新绘制的字形。
