...
afterEach(cleanup)
import { AppRoot } from '@truly-private/omdsh-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@truly-private/omdsh-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@truly-private/omdsh-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@truly-private/omdsh-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
