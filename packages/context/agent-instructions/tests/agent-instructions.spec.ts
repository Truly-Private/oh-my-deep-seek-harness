...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@truly-private/omdsh-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@truly-private/omdsh-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@truly-private/omdsh-agent'
import AgentLoop from '@truly-private/omdsh-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@truly-private/omdsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import ToolRuntime, { defineContentToolFixture } from '@truly-private/omdsh-tools'
import type {
  ToolExecution,
  ToolExecutionToken,
...
} from '@truly-private/omdsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@truly-private/omdsh-agent-instructions')
