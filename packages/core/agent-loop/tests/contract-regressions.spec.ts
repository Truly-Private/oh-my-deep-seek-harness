...
import LlmRuntime, { createUserMessage, CallId, LlmError, MessageSource, ProviderRequestId, StreamChunk  } from '@truly-private/omdsh-llm'
import SessionStore, { Session, SessionEvent, SessionId, TurnEndReason, type UserMessage } from '@truly-private/omdsh-session'
import SystemPrompt from '@truly-private/omdsh-system-prompt'
import ToolRuntime, { defineContentToolFixture, type PostToolDecision } from '@truly-private/omdsh-tools'
import AgentRegistry, { type Agent } from '@truly-private/omdsh-agent'
...
describe('turn and step boundary recovery', () => {
  ...
  it('a step/end validation failure surfaces the resulting open-step invariant', async () => {
    ...
    expect(errors.map(error => error.message)).toEqual([
      ...
      'invariant violated by "@truly-private/omdsh-session": turn/end 1 while step 1 is still open',
    ])
    expect(boundaryCounts(agent)).toMatchObject({
      turnStart: 1,
      turnEnd: 0,
