...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@truly-private/omdsh-tool-call-timeout-policy` wrapper.
