 ...
 * execution pipeline.
 * @module @truly-private/omdsh-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@truly-private/omdsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@truly-private/omdsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@truly-private/omdsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@truly-private/omdsh-llm'
import type { Agent } from '@truly-private/omdsh-agent'
import { snapshotJsonValue } from '@truly-private/omdsh-session'
import type { JsonValue, UserMessage } from '@truly-private/omdsh-session'
import type { ToolProviderResult } from '@truly-private/omdsh-system-prompt'
import type { CodeRuntime } from '@truly-private/omdsh-code-runtime'
...
export type { JsonValue } from '@truly-private/omdsh-session'
...
// lives in its own UI-facing module; re-export it so `@truly-private/omdsh-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@truly-private/omdsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@truly-private/omdsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@truly-private/omdsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@truly-private/omdsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@truly-private/omdsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @truly-private/omdsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
