...
// Load it alongside a ctx.fs provider (e.g. @truly-private/omdsh-fs-local) and the
// @truly-private/omdsh-tool-fs tools; the tools dispatch the fs/* events this plugin
...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@truly-private/omdsh-tool-fs` | model-facing schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@truly-private/omdsh-fs-observation-policy` (this) | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@truly-private/omdsh-fs` | `ctx.fs`: text IO + atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@truly-private/omdsh-fs-local` | local implementation of `ctx.fs` |

## How the gate participates

Three `fs/*` events (declared by `@truly-private/omdsh-fs`, dispatched by `@truly-private/omdsh-tool-fs`):
...
Because the plugin influences the world only through events, removing it does not break `@truly-private/omdsh-tool-fs` at a service-injection boundary: the tool falls through to the bare `ctx.fs` provider (unconditional write/edit, no observed-state). Loading it back layers the policy on. That graceful add/remove is the whole point of the event gate over a mandatory method service.
