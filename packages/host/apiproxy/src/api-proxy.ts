...
import { installModelSelection } from '@truly-private/omdsh-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@truly-private/omdsh-agent'
import type {} from '@truly-private/omdsh-agent-presets/types'
import { AttachmentError } from '@truly-private/omdsh-attachment'
import type { ImageAttachmentRef } from '@truly-private/omdsh-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@truly-private/omdsh-llm'
import { errorChain } from '@truly-private/omdsh-llm'
import type { ContentBlock, MessageSource } from '@truly-private/omdsh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@truly-private/omdsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@truly-private/omdsh-session'
import type { SessionPersistence } from '@truly-private/omdsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@truly-private/omdsh-session-query'
import { SubagentError } from '@truly-private/omdsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@truly-private/omdsh-subagent'
import { isUserInvocable } from '@truly-private/omdsh-skill'
import type { Workspace, WorkspaceRecord } from '@truly-private/omdsh-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@truly-private/omdsh-workspace'
...
} from '@truly-private/omdsh-agent-presets'
import type { PresetBearingSession } from '@truly-private/omdsh-agent-presets'
...
import type { SessionRawArtifact } from '@truly-private/omdsh-session-persistence'
import {
...
import type {} from '@truly-private/omdsh-jobs'
import type { JobSnapshot } from '@truly-private/omdsh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@truly-private/omdsh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@truly-private/omdsh-goal'
import type { GoalRef as CoreGoalRef } from '@truly-private/omdsh-goal'
...
import type {} from '@truly-private/omdsh-cordis-host-runner/types'
import type {} from '@truly-private/omdsh-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@truly-private/omdsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@truly-private/omdsh-settings'
import { credentialRef } from '@truly-private/omdsh-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@truly-private/omdsh-session-title'
import type { CallId } from '@truly-private/omdsh-llm/brand'
import type { ScopeKey } from '@truly-private/omdsh-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@truly-private/omdsh-user-approval'
...
} from '@truly-private/omdsh-user-questions'
import { UserQuestionError } from '@truly-private/omdsh-user-questions'
import { DirectoryPickerError } from '@truly-private/omdsh-host-directory-picker'
import {
...
} from '@truly-private/omdsh-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @truly-private/omdsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @truly-private/omdsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @truly-private/omdsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @truly-private/omdsh-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @truly-private/omdsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @truly-private/omdsh-skill', details: {} })
        }
