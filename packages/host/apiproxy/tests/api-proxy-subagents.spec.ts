...
import type { SessionEvent, SessionHeader, SessionId } from '@truly-private/omdsh-session'
...
describe('subagent gateway', () => {
  ...
  it('maps the missing projections capability to one wire face on list, history, and prompt', async () => {
    const listError = () => new SubagentError(
      'listing subagents requires the sessionProjections registry (load @truly-private/omdsh-session-projection)',
