...
import { bindScopeParent, createScope, scopeOf } from '@truly-private/omdsh-scope'
import type { ScopeKey } from '@truly-private/omdsh-scope'
import { JobId } from '@truly-private/omdsh-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@truly-private/omdsh-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @truly-private/omdsh-tool-jobs in its composition)')
  })
