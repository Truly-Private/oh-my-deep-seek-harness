 ...
 * process-local registry lives in `@truly-private/omdsh-jobs-local`.
 * @module @truly-private/omdsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@truly-private/omdsh-jobs is the abstract job registry seam; load an implementation such as @truly-private/omdsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
