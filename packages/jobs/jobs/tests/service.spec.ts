...
import { JobId, JobRegistry } from '@truly-private/omdsh-jobs'
import type {
 ...
 * behavior suite lives with `@truly-private/omdsh-jobs-local`.
 */
...
describe('JobRegistry seam', () => {
  ...
  it('mounting the abstract seam directly fails loudly at load (stale-composition fence)', async () => {
    ...
    await expect(ctx.plugin(JobRegistry as unknown as typeof StubJobRegistry))
      .rejects.toThrow(/abstract job registry seam; load an implementation such as @truly-private\/omdsh-jobs-local/)
  })
})
