...
/**
 * Keyless built-artifact smoke: plain Node imports `@truly-private/omdsh-lsp` and
 * `@truly-private/omdsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@truly-private/omdsh-lsp')
      const LspLocal = await import('@truly-private/omdsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@truly-private/omdsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@truly-private/omdsh-subprocess-local')
