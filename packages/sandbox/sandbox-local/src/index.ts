 ...
 * @module @truly-private/omdsh-sandbox-local
 */
...
import { assertNever } from '@truly-private/omdsh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@truly-private/omdsh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@truly-private/omdsh-sandbox'
import type { SessionId } from '@truly-private/omdsh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@truly-private/omdsh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@truly-private/omdsh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@truly-private/omdsh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@truly-private/omdsh-sandbox-windows-acl/src/runner.ts'))
