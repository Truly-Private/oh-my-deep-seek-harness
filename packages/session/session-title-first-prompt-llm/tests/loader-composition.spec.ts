...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@truly-private/omdsh-llm'
import type { GenerateOptions, StreamChunk } from '@truly-private/omdsh-llm'
import SessionStore, { SessionId } from '@truly-private/omdsh-session'
import SessionTitleService from '@truly-private/omdsh-session-title'
import * as providerPlugin from '@truly-private/omdsh-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "- name: '@truly-private/omdsh-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@truly-private/omdsh-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@truly-private/omdsh-session-title', SessionTitleService],
    ['@truly-private/omdsh-session-title-first-prompt-llm', providerPlugin],
  ])
