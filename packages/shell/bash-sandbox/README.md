...
Sandbox-consuming Service Provider for the [`@truly-private/omdsh-shell`](../shell/) executor seam. Load it **instead of** `@truly-private/omdsh-bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@truly-private/omdsh-sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `dsh-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
...
```yaml
- id: sandbox
  name: '@truly-private/omdsh-sandbox-local'
- id: sandbox-policy
