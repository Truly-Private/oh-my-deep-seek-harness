...
| Package | Role |
|---|---|
| `@truly-private/omdsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@truly-private/omdsh-bash-local` | Service Provider: local subprocesses |
| `@truly-private/omdsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@truly-private/omdsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
