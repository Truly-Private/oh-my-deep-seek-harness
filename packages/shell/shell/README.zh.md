...
| 包 | 职责 |
|---|---|
| `@truly-private/omdsh-shell`（本包） | Service Definition：抽象服务 + 词汇类型 |
| `@truly-private/omdsh-bash-local` | Service Provider：本地子进程 |
| `@truly-private/omdsh-bash-sandbox` | Service Provider：沿用 `dsh-bash-local` 的机制，但通过 [`ctx.sandbox`](../../sandbox/sandbox/) 限制每次 spawn，并将拒绝报告为结果事实 |
| `@truly-private/omdsh-tool-bash` | 基于 `ctx.shell`、面向模型的工具 schema |
