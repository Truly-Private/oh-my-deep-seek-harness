...
需要加载执行器 Service Provider（例如 `@truly-private/omdsh-bash-local`）与 [`@truly-private/omdsh-shell-env`](../shell-env/README.zh.md) 注册表；在每个注入服务就绪之前，插件会保持等待状态（`inject: ['tools', 'bash', 'systemPrompt', 'bashEnv']`）。工具约定是 bash 方言——请挂载能解析 bash 的执行器。
