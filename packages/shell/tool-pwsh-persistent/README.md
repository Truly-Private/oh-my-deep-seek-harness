...
The generated [`pwsh` schema](../../../docs/tool-catalog.md#truly-privateomdsh-tool-pwsh-persistent), including the configured `description`. The plugin contributes no standalone system-prompt section; the deployment owns persona and environment guidance.
...
- A model redefinition of the `prompt` function removes owned marker readiness. A provider without foreground evidence may still use silence; a provider with foreground evidence requires post-write output or a foreground transition and can time out when a fast command completes entirely between polls without emitting output.
