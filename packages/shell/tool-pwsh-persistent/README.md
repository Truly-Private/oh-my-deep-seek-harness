...
- A model redefinition of the `prompt` function removes owned marker readiness. A provider that cannot prove foreground stdin waits may still use silence; a provider that proved a pre-write wait requires foreground activity and can time out when a fast command completes entirely between polls.
