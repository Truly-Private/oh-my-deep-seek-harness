...
This package implements one skill source. It scans local project, custom, and user skill roots, parses `SKILL.md` or flat Markdown skill files, and registers the provider on `ctx.skills`. The registry remains in `@truly-private/omdsh-skill`; the durable session catalogs and model-facing loader tool remain in `@truly-private/omdsh-tool-skill`.
...
| `dshHome` | `$DSH_HOME` or `~/.dsh` | DeepSeek Harness config root resolved by [`@truly-private/omdsh-home-paths`](../../util/home-paths/README.md); scans `skills` under this directory. |
