...
| Package | Role |
|---|---|
| `@truly-private/omdsh-spill` (this) | Service Definition: abstract service + vocabulary types |
| `@truly-private/omdsh-spill-local` | Service Provider: private session-scoped files on the host filesystem |
| `@truly-private/omdsh-spill-policy` | Consumer: the tool-result policy that spills oversized final results |
...
Storage is grouped by the request's `owner` session as a save-time namespace; the backend chooses its own private representation and may derive names from — never trust as a path — the caller's `suggestedName`. The seam owns storage only: NO retention policy (that is [`@truly-private/omdsh-output-retention`](../../util/output-retention)), NO tool-result replacement (that is `@truly-private/omdsh-spill-policy`), NO retrieval/search API (the backend's `retrievalHint` tells the model what to do with the locator).
