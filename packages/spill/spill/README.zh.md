...
| 包 | 职责 |
|---|---|
| `@truly-private/omdsh-spill`（本包） | Service Definition：抽象服务与词汇类型 |
| `@truly-private/omdsh-spill-local` | Service Provider：位于宿主文件系统中的私有会话级文件 |
| `@truly-private/omdsh-spill-policy` | Consumer：对过大最终结果执行 spill 的工具结果策略 |
...
存储操作以请求的 `owner` 会话作为保存时命名空间进行分组；后端自行选择私有表示，并可以从调用方的 `suggestedName` 派生名称，但绝不能将其当作可信路径。该 seam 只负责存储：不提供保留策略（由 [`@truly-private/omdsh-output-retention`](../../util/output-retention) 负责），不替换工具结果（由 `@truly-private/omdsh-spill-policy` 负责），也不提供取回/搜索 API（后端的 `retrievalHint` 会告诉模型如何使用定位信息）。
