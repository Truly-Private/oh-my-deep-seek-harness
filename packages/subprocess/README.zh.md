...
| 包 | ctx 键 | 角色 |
|---|---|---|
| [`subprocess`](subprocess/README.md)（`@truly-private/omdsh-subprocess`） | `ctx.subprocess` | Service Definition：可执行文件查找、普通受管 spawn、终端进程原语、句柄生命周期，以及共享的环境／输出词汇 |
| [`subprocess-local`](subprocess-local/README.md)（`@truly-private/omdsh-subprocess-local`） | 无 | 本地 Service Provider：detached 进程树、有界收集／spill、`node-pty`、前台／会话检查、进程树信号发送，以及先终止再等待退出的 dispose（资源释放） |
