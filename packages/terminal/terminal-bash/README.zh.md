...
就绪检测结合以下机制：由前台状态验证的私有提示符标记、提供方报告的前台 stdin 等待事实、静默回退和绝对超时。只有最新自有标记之后的可打印尾部与本次 send 的预期提示符文本完全相等，标记才算就绪；默认值是后端受控提示符，拥有替换提示符的消费方也可声明确切值。OSC 标记与提示符文本跨数据回调拆分时仍受支持。提供方写入前收集的提示符与静默证据，包括写入前前台检查仍在等待时收集的证据，都会在写入边界丢弃。当提供方能够给出前台证据时，已提交输入的 send 只有在写入后活动把后续标记或静默结果归属于该 send 时才会接受它：前台进程组变化、脱离写入前 stdin 等待、成功中断、更早出现的可打印输出，或同一回调中位于标记之前的可打印文本。轮询首次观察到这种前台活动时，静默推断会重新开始一个完整的 `idleSilenceMs` 时间窗，因此延迟轮询不会使用确认该活动之前已经累计的静默时间。进程组在写入前已处于忙碌状态，本身不能证明新输入已经运行；如果它在两次轮询之间回到 stdin 等待，则必须观察到写入后输出。因此，来自前一 send 的延迟标记与提示符不能使当前 send 完成。无法提供前台证据的提供方仍保留静默回退。如果 bash 在终端提供方发布其重新取得前台进程组的状态前打印一个已接受标记，轮询会在普通静默上限之后再保留该候选状态 `handoffGraceMs`，使恰好同时发生的前台交接有机会胜出。未知的前台状态绝不会作为精确空闲的正向信号。尚未发布的启动过程中，回退路径要求已经观察到输出；零输出静默不能发布空会话，超时则拒绝 spawn。取消操作会关闭尚未发布的 shell，并以调用方提供的确切中止原因拒绝；`TerminalBackendCleanupError` 会单独保留清理失败。调用方的 signal 会转发给终端分配与就绪初始化；发布后，句柄负责其生命周期。未完成的终端控制序列受 `maxReadBytes` 限制；超过上限后，系统会丢弃内容直到其终止符。格式错误的 UTF-8 终端输出使用替换字符；末尾的回车会跨回调保留，使拆分的 CRLF 合并为一个换行。
...
策略归属方会贡献与具体能力无关的 `sandbox:policy` 上下文。模型通过 `@truly-private/omdsh-tool-terminal` 或其他 PTY 消费方还可能收到有界的 MOTD、发送增量、scrollback 页、就绪原因和清理错误。
