...
import AgentRegistry, { Inbox, type Agent } from '@truly-private/omdsh-agent'
...
import SandboxPolicyService, { setSandboxMode } from '@truly-private/omdsh-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@truly-private/omdsh-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@truly-private/omdsh-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@truly-private/omdsh-terminal-bash'
import { ENCODING_PREAMBLE } from '@truly-private/omdsh-pwsh-local'
import * as ptyLocal from '@truly-private/omdsh-terminal-bash'
import type { ResolvedConfig } from '@truly-private/omdsh-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@truly-private/omdsh-terminal-bash/src/session.ts'
import { SubprocessRuntime } from '@truly-private/omdsh-subprocess'
import type {
  SubprocessHandle,
  SubprocessSpawnSpec,
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      initialize: async () => {
        initialized = true
        session.motd = 'PS /workspace> '
      },
      startSend: (request: TerminalSendRequest) => {
        sends.push(request)
        const promptReady = sends.length > 1
        return {
          done: Promise.resolve({
            viewport: promptReady ? 'dsh> ' : ENCODING_PREAMBLE + PWSH_PROMPT_SETUP,
            waitReason: promptReady ? 'stdin_read' as const : 'inferred_idle' as const,
  ...
  it('keeps waiting for the marker prompt when the first send settles on silence', async () => {
    ...
    const session = {
      motd: '',
      initialize: async () => { session.motd = 'PS /workspace> ' },
      startSend: (request: TerminalSendRequest) => {
        sends.push(request)
        const prompt = sends.length > 1
        return {
          done: Promise.resolve({
            viewport: prompt ? 'dsh> ' : 'PowerShell 7.6.4\n',
            waitReason: prompt ? 'stdin_read' as const : 'inferred_idle' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
  ...
  it('rejects a pwsh bootstrap whose shell exits or times out', async () => {
    ...
    await ctx.plugin(EmptySandbox)
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access', workspaceRoot: '/workspace' })
    const sessionFor = (waitReason: TerminalWaitReason, initializeError?: Error): LocalPtySession => ({
      motd: '',
      initialize: async () => {
    ...
    const exited = new BashTerminalBackend(
      ctx,
    ...
    const timedOut = new BashTerminalBackend(
      ...
      { ...config(), shellDialect: 'pwsh' },
      async () => terminalHandle(),
      () => sessionFor('timeout', new Error('PTY shell did not reach readiness before startup timeout')),
    )
