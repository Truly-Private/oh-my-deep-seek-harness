...
import { LocalPtySession } from '@truly-private/omdsh-terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@truly-private/omdsh-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@truly-private/omdsh-terminal'
import type {
  SubprocessOutcome,
  SubprocessTerminalHandle,
  SubprocessTerminalSignal,
} from '@truly-private/omdsh-subprocess'
import { TerminalError } from '@truly-private/omdsh-terminal'
import type {
  ProcessIdentity,
  ProcessInspector,
} from '@truly-private/omdsh-subprocess-local/src/process-inspector.ts'
...
describe('LocalPtySession readiness and output', () => {
  ...
  it('discards prompt readiness observed during asynchronous pre-write inspection', async () => {
    ...
    terminal.emitData('long-running-command\r\n\x1b]133;D;0\x07dsh> ')
    await vi.advanceTimersByTimeAsync(10)
    expect((await operation.done).waitReason).toBe('stdin_read')
  })
  ...
  it('accepts the prompt text declared by the send after same-chunk command output', async () => {
    vi.useFakeTimers()
    const terminal = new FakeTerminal()
    const inspector = new FakeInspector()
    ...
    const operation = session.startSend({
      text: 'install custom prompt',
      submit: true,
      expectedPromptText: 'custom> ',
    })
    await Promise.resolve()
    await Promise.resolve()
    let settled = false
    void operation.done.then(() => { settled = true })
    terminal.emitData('\x1b]133;D;0\x07stale> ')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)
    terminal.emitData('install custom prompt\r\n\x1b]133;D;0\x07custom> ')
    await vi.advanceTimersByTimeAsync(10)
