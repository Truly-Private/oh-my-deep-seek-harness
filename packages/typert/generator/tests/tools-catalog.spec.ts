...
import type { TypertContribution } from '@truly-private/omdsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@truly-private/omdsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@truly-private/omdsh-tools'],
    }).analyze()
    ...
    const artifact = new FaceModelEmitter(host).emit('@truly-private/omdsh-tools')
    ...
    const record = ctx.typert.getPackage('@truly-private/omdsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@truly-private/omdsh-tools', 'host')).toBeUndefined()
  })
})
