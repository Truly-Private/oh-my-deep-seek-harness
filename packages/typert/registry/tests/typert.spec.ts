...
} from '@truly-private/omdsh-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@truly-private/omdsh-typert-protocol'
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@truly-private/omdsh-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@truly-private/omdsh-tools', 'ToolInput')).toBe('@truly-private/omdsh-tools#ToolInput')
    expect(typertPackageKey('@truly-private/omdsh-tools', 'host')).toBe('@truly-private/omdsh-tools#host')
    expect(ctx.typert.get('@truly-private/omdsh-tools#ToolInput')).toMatchObject({
      package: '@truly-private/omdsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@truly-private/omdsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@truly-private/omdsh-tools', 'host')).toMatchObject({
      key: '@truly-private/omdsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@truly-private/omdsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@truly-private/omdsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@truly-private/omdsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@truly-private/omdsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@truly-private/omdsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@truly-private/omdsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@truly-private/omdsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@truly-private/omdsh-tools#ToolInput')).not.toBe(projected)
  })
