...
| Package | Role |
|---|---|
| `@truly-private/omdsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@truly-private/omdsh-web-search-exa` | Search provider: Exa |
| `@truly-private/omdsh-web-search-perplexity` | Search provider: Perplexity |
| `@truly-private/omdsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@truly-private/omdsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
