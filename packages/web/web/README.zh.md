...
| 包 | 职责 |
|---|---|
| `@truly-private/omdsh-web`（本包） | Service Definition：服务、提供方注册表、选择策略、请求／结果词汇、`WebError` 分类体系 |
| `@truly-private/omdsh-web-search-exa` | 搜索提供方：Exa |
| `@truly-private/omdsh-web-search-perplexity` | 搜索提供方：Perplexity |
| `@truly-private/omdsh-web-fetch-http` | 抓取提供方：匿名公共 HTTP(S) |
| `@truly-private/omdsh-tool-web` | Consumer：面向模型的 `web_search`／`web_fetch` 工具 schema，构建于 `ctx.web` 之上 |
