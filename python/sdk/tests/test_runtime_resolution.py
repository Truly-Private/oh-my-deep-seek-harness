...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@truly-private/omdsh-agent-spine-demo" in config
    assert "@truly-private/omdsh-session-persistence-jsonl" in config
    assert "@truly-private/omdsh-session-checkpoint-policy" in config
