...
function clientConfigs(id = '@truly-private/omdsh-client-test') {
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    ...
    expect(resolveId('@truly-private/omdsh-client-web-react')).toBeNull()
  ...
  it('throws on any other harness package leak', () => {
  ...
  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    ...
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@truly-private/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@truly-private/omdsh-client-runtime/client'])
  })
})
