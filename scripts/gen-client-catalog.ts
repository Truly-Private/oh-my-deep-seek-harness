...
/** Drop the `@truly-private/omdsh-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@truly-private/omdsh-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @truly-private/omdsh-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
