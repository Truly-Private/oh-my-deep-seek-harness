...
const SESSION_PACKAGE = '@truly-private/omdsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@truly-private/omdsh-session/types'
 ...
 * top-level declaration (in `@truly-private/omdsh-session`) and any declaration
 * merge inside a `declare module '@truly-private/omdsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @truly-private/omdsh-session/known-event-types',
    ' */',
    '',
    '/**',
