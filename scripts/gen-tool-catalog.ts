...
import type { ToolSchema } from '@truly-private/omdsh-llm'
import AgentRegistry from '@truly-private/omdsh-agent'
import type { Agent } from '@truly-private/omdsh-agent'
import { createScope } from '@truly-private/omdsh-scope'
...
import GoalService from '@truly-private/omdsh-goal'
import SystemPrompt from '@truly-private/omdsh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@truly-private/omdsh-tools'
import LocalBashExecutor from '@truly-private/omdsh-bash-local'
import * as BashEnvPlugin from '@truly-private/omdsh-shell-env'
import { PwshLocalExecutor } from '@truly-private/omdsh-pwsh-local'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@truly-private/omdsh-attachment'
import UserQuestionService from '@truly-private/omdsh-user-questions'
import PlanModeController from '@truly-private/omdsh-plan-mode'
import WebRuntime from '@truly-private/omdsh-web'
import * as WebSearchExa from '@truly-private/omdsh-web-search-exa'
import * as WebFetchLocal from '@truly-private/omdsh-web-fetch-http'
import SubagentRuntime from '@truly-private/omdsh-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@truly-private/omdsh-subagent'
import * as ToolSubagentControl from '@truly-private/omdsh-tool-subagent-control'
import * as ToolSubagentListAgents from '@truly-private/omdsh-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@truly-private/omdsh-tool-subagent-report'
import SkillRegistry from '@truly-private/omdsh-skill'
import * as SkillFileSystem from '@truly-private/omdsh-skill-filesystem'
import LocalJobRegistry from '@truly-private/omdsh-jobs-local'
import * as ToolAskUser from '@truly-private/omdsh-tool-ask-user'
import * as ToolBash from '@truly-private/omdsh-tool-bash'
import * as ToolPwsh from '@truly-private/omdsh-tool-pwsh'
import * as ToolBashPersistent from '@truly-private/omdsh-tool-bash-persistent'
import * as ToolPwshPersistent from '@truly-private/omdsh-tool-pwsh-persistent'
import CordisHostRunner from '@truly-private/omdsh-cordis-host-runner'
import * as ToolCordis from '@truly-private/omdsh-tool-cordis'
import * as ToolFs from '@truly-private/omdsh-tool-fs'
import * as ToolFsSearch from '@truly-private/omdsh-tool-fs-search'
import * as ToolStrReplaceEditor from '@truly-private/omdsh-tool-str-replace-editor'
import TerminalSessionService from '@truly-private/omdsh-terminal'
import * as ToolPty from '@truly-private/omdsh-tool-terminal'
import * as ToolGoal from '@truly-private/omdsh-tool-goal'
import * as ToolSchedule from '@truly-private/omdsh-schedule'
import Lsp from '@truly-private/omdsh-lsp'
import * as ToolLsp from '@truly-private/omdsh-tool-lsp'
import * as ToolSkill from '@truly-private/omdsh-tool-skill'
...
import type TeamService from '@truly-private/omdsh-experimental-agent-team'
import * as ToolTeam from '@truly-private/omdsh-experimental-tool-agent-team'
import * as ToolTodo from '@truly-private/omdsh-tool-todo'
import * as ToolSubagent from '@truly-private/omdsh-tool-subagent'
import * as ToolWeb from '@truly-private/omdsh-tool-web'
import VmWorkflowEngine from '@truly-private/omdsh-workflow-worker-thread'
import * as ToolRalph from '@truly-private/omdsh-tool-ralph'
import * as ToolWorkflow from '@truly-private/omdsh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@truly-private/omdsh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  {
    pkg: '@truly-private/omdsh-plan-mode',
    dir: 'plan-mode',
  ...
  {
    pkg: '@truly-private/omdsh-tool-bash',
    dir: 'tool-bash',
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@truly-private/omdsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@truly-private/omdsh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@truly-private/omdsh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@truly-private/omdsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@truly-private/omdsh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@truly-private/omdsh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@truly-private/omdsh-tool-bash-persistent',
  ...
  {
    pkg: '@truly-private/omdsh-tool-pwsh-persistent',
  ...
  {
    pkg: '@truly-private/omdsh-tool-str-replace-editor',
  ...
  {
    pkg: '@truly-private/omdsh-tool-fs',
    dir: 'tool-fs',
    ...
    note:
      'The read-before-write/edit policy is added by `@truly-private/omdsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@truly-private/omdsh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@truly-private/omdsh-tool-terminal',
    dir: 'tool-terminal',
  ...
  {
    pkg: '@truly-private/omdsh-tool-goal',
    dir: 'tool-goal',
  ...
  {
    pkg: '@truly-private/omdsh-schedule',
    dir: 'schedule',
  ...
  {
    pkg: '@truly-private/omdsh-tool-lsp',
    dir: 'tool-lsp',
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@truly-private/omdsh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@truly-private/omdsh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@truly-private/omdsh-tool-skill',
    dir: 'tool-skill',
  ...
  {
    pkg: '@truly-private/omdsh-tool-session-query',
  ...
  {
    pkg: '@truly-private/omdsh-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@truly-private/omdsh-tool-subagent-control',
  ...
  {
    pkg: '@truly-private/omdsh-tool-subagent-report',
  ...
  {
    pkg: '@truly-private/omdsh-tool-jobs',
    dir: 'tool-jobs',
  ...
  {
    pkg: '@truly-private/omdsh-experimental-tool-agent-team',
    dir: 'tool-agent-team',
  ...
  {
    pkg: '@truly-private/omdsh-tool-todo',
    dir: 'tool-todo',
  ...
  {
    pkg: '@truly-private/omdsh-tool-workflow',
    dir: 'tool-workflow',
  ...
  {
    pkg: '@truly-private/omdsh-tool-web',
    dir: 'tool-web',
