...
function fixture(options: {
  ...
  const packageName = options.packageName ?? '@truly-private/omdsh-probe'
  const manifest = {
    name: packageName,
    ...
    peerDependencies: options.invariantDependency === false ? {} : {
      '@truly-private/omdsh-invariants': 'workspace:^',
    },
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) => {
  ctx.invariants.register('@truly-private/omdsh-foreign', install)
...
`
  ...
  it('rejects generated markers and reporter-free executable installers', () => {
    const generated = fixture({
      source: `/** @generated */\n${handwrittenInvariant('@truly-private/omdsh-probe')}`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) =>
  Promise.resolve(ctx.invariants.register('@truly-private/omdsh-probe', install))
`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
  ctx.invariants.register('@truly-private/omdsh-probe', () => {})
`,
    })
  ...
  it.each([
    ...
    const source = `${handwrittenInvariant('@truly-private/omdsh-probe')}\n${defaultExport}\n`
...
export const name = 'probe-invariant'
export const inject = ['invariants']
const PACKAGE_NAME = '@truly-private/omdsh-probe'
