...
const RELEASE_ENTRY_PACKAGE = '@truly-private/omdsh'
...
class WorkspacePackageSet {
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      const expectedScope = isVendored ? '@deepseek-ai/' : '@truly-private/'
      if (!name.startsWith(expectedScope)) {
        throw new Error(`${manifestPath} must name a ${expectedScope} package`)
      }
      if (name === '@truly-private/omdsh-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@truly-private/omdsh/lib/bin.js')
