...
describe('DSH package license gate', () => {
  it('checks root, unhyphenated CLI, and dsh-prefixed package names while ignoring other families', () => {
    ...
    writeManifest(root, 'apps/cli/package.json', { name: '@truly-private/omdsh', license: 'MIT' })
    ...
    expect(inspectDshPackageLicenses(root)).toEqual({
      packageCount: 3,
      failures: [
        'packages/core/agent/package.json: @truly-private/omdsh-agent must declare "license": "MIT"; found "BSD-3-Clause".',
      ],
    })
  })
  ...
  it('rejects a missing license declaration', () => {
    const root = createWorkspace()
    writeManifest(root, 'packages/core/agent/package.json', { name: '@truly-private/omdsh-agent' })

    expect(inspectDshPackageLicenses(root).failures).toEqual([
      'packages/core/agent/package.json: @truly-private/omdsh-agent must declare "license": "MIT"; found undefined.',
    ])
  })
})
